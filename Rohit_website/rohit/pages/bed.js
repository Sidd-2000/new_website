        ...
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).map((item) => {
              return <Link passHref={true} keys={products[item]._id}  href={`/{slug}}`}><div
               className="lg:w-auto md:w-1/2 p-4 w-full shadow-lg m-5 cursor-pointer">
                    <img
                      alt="ecommerce"
                      className="m-auto h-[20vh] md:h-[36vh] w-full flex"
                      src={products[item].img}/>
                  <div className="mt-4 text-center md:text-left">
                    <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">bed</h3>
...
export async function getServerSideProps(context) {
  if (!mongoose.connections[0].readyState) {
    ...
    for (let item of products) {
      if (item.title in beds) {
        if (
          beds[item.title].color.includes(item.color) &&
          item.availableQty > 0
        ) {
  ...
  return {
    props: { products: JSON.parse(JSON.stringify(beds)) }, // will be passed to the page component as props
  };
}
